...
local STREAM_RETARGET_MAX_ANGLE_OFFSET = math.pi / 36
...
local function sample_stream_retarget_position(job, tick)
  ...
  local base_x = job.stream_base_direction_x or job.stream_direction_x or 1
  local base_y = job.stream_base_direction_y or job.stream_direction_y or 0
...
function Launcher.enqueue(surface, center, target, distance, node, speed, force, cause, charge_count, ensure_tick_handler)
  ...
  append_pending_job {
    ...
    stream_base_direction_x = direction_x,
    stream_base_direction_y = direction_y,
